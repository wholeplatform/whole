 ...
 */
public class DataEntityTableFigure extends ContentPaneFigure {
	private EntityLabel keyword;

	public DataEntityTableFigure() {
		...
		EntityFigure row = new EntityFigure(new RowLayout().withSpacing(8));
		row.add(createContentPane(3));
		keyword = row.addContentLighter("datatype");
		add(row);
	}

	public void showKeyword(boolean visible) {
		keyword.setVisible(visible);
	}
}
