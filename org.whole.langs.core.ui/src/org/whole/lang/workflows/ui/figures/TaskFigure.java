 ...
 */
public class TaskFigure extends AbstractActivityWithTableFigure {
	public TaskFigure() {
		super(4, NotationImages.INPUT_TASK16);

		TableRowFigure row = new TableRowFigure();
		row.addLabel("description");
		row.add(createContentPane(1));
		contentFigure.add(row);

		row = new TableRowFigure();
		row.addLabel("assignments");
		row.add(createContentPane(2, new MarginBorder(3,0,3,0)));
		contentFigure.add(row);

		row = new TableRowFigure();
		row.addLabel("factory");
	...
	public void paintFigure(Graphics g) {
        ...
        PaintFigureHelper.drawFilledRoundRectangle(g, getBounds(),
        		ColorConstants.darkGray, FigureUtilities.lighter(ColorConstants.yellow));
	}
}
