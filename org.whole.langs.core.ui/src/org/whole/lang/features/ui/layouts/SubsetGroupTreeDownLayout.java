 ...
 */
public class SubsetGroupTreeDownLayout extends FeatureGroupTreeDownLayout implements IFeatureGroupLayout{
	{
		CHILDREN_GAP = 8;
	}
	...
	protected ConnectionAnchor[] createRootTargetAnchors(IStyledFigure styledFigure) {
		return new ConnectionAnchor[] {
				AnchorFactory.createTopMiddleAnchor(childFigure[0])
		};
	}

	@Override
	public Point getRootPoint() {
		return childFigure[0].getBounds().getTop();
	}
	...
	public void paintConnections(Graphics graphics, Point... children) {
		...
		if (children.length > 1) {
			...
			double b0 = Math.abs(children[0].x - root.x);
			double h0 = children[0].y - root.y;
			int arc0 = (int) Math.toDegrees(Math.atan2(h0, b0));

			double b1 = Math.abs(children[children.length-1].x - root.x);
			double h1 = children[children.length-1].y - root.y;
			int arc1 = (int) Math.toDegrees(Math.atan2(h1, b1));

			if (useDefaultLayout) {
				graphics.drawArc(
						root.x-ARC_SIZE/2, root.y-ARC_SIZE/2, ARC_SIZE, ARC_SIZE,
						180 + arc0, 180 - arc0 - arc1);
			} else {
