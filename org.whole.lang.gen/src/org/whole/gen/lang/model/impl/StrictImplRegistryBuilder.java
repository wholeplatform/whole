 ...
 */
public class StrictImplRegistryBuilder extends CompilationUnitBuilder {
	public final MethodDeclaration createPrototypesMethod;

	public StrictImplRegistryBuilder(LanguageGenerator generator) {
		super(generator);
		
		addClassDeclaration(generator.strictImplRegistryName(), generator.implRegistryName());
...
//						newFieldAccess(generator.specificEntityDescriptorEnumName(), "instance")));
		...
		createPrototypesMethod.getBody().statements().add(
				newExpressionStatement(newSuperMethodInvocation("createPrototypes")));
		...
		createPrototypesMethod.getBody().statements().add(
				newExpressionStatement(
						newMethodInvocation("put", newClassInstanceCreation(
								generator.entityStrictImplName(entityName)))));
	}
}
