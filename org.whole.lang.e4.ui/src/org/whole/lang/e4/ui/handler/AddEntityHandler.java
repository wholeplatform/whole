 ...
 */
public class AddEntityHandler extends TypedModelTransactionHandler {
	public boolean isEnabled(IBindingManager bm) {
		return HandlersBehavior.canAddEntity(bm);
	}
	public void run(IBindingManager bm) {
		HandlersBehavior.addEntity(bm);
	}
	public String getLabel(IBindingManager bm) {
		EntityDescriptor<?> ed = (EntityDescriptor<?>) bm.wGetValue("entityDescriptor");
		return "add "+ StringUtils.camelCaseToSpacedWords(ed.getName());
	}
}
