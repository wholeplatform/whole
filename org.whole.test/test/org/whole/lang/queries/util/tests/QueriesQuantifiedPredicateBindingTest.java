...
public class QueriesQuantifiedPredicateBindingTest extends TestCase {
    ...
    protected static IEntity create(String templateName) {
        return QueriesQuantifiedPredicateBindingTestTemplateManager.instance().create(templateName);
    }
     ...
     */
    @Test
    public void testQuantifiedPredicateBindings() {
        ITransactionScope ts = BindingManagerFactory.instance.createTransactionScope();
        try {
            bindings().wEnterScope(ts);
            IEntity subject;
            subject = evaluate("fragment1");
            assertThat("at /testCases/0/tests/0/body/0", subject, matches(evaluate("fragment2")));
            subject = evaluate("fragment3");
            assertThat("at /testCases/0/tests/0/body/1", subject, matches(evaluate("fragment4")));
            subject = evaluate("fragment5");
            assertThat("at /testCases/0/tests/0/body/2", subject, matches(evaluate("fragment6")));
            subject = evaluate("fragment7");
            assertThat("at /testCases/0/tests/0/body/3", subject, matches(evaluate("fragment8")));
            subject = evaluate("fragment9");
            assertThat("at /testCases/0/tests/0/body/4", subject, matches(evaluate("fragment10")));
            subject = evaluate("fragment11");
            assertThat("at /testCases/0/tests/0/body/5", subject, matches(evaluate("fragment12")));
        } finally {
            ts.rollback();
            bindings().wExitScope();
        }
